...
# }
   ...
   foreach my $i (0 .. $validation_batch - 1) {
      ...
      $CNN->results($last_activated_output) ;
say Dumper($last_activated_output);
      my $max_calc_idx = argmax($last_activated_output);
say "max_calc_idx = " . Dumper($max_calc_idx);
      my $max_target_idx = argmax($v_labels[$i]);
say "success = calc = " . $max_calc_idx->[0] . " == target = " . $max_target_idx->[0];# if $debug == 1;
