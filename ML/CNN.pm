...
# convolution 1: input to c1 for p kernels
   ...
   foreach my $p (0 .. $self->{c1_kernels} - 1) {
      foreach my $i (0 .. $self->{input_dimensions}[0] - 1 - $self->{range_upper}) { # when no padding is applied, we need to end each row half a kernel width before the end
         next if ($i < $self->{range_upper}); # when no padding is applied, we need to start each row half a kernel width in
         foreach my $j (0 .. $self->{input_dimensions}[1] - 1 - $self->{range_upper}) { # when no padding is applied, we need to end each row half a kernel width before the end
            next if ($j < $self->{range_upper}); # when no padding is applied, we need to start each column half a kernel width in
            $self->{c1}[$p][$i - $self->{range_upper}][$j - $self->{range_upper}] = 0;
         }
      }
   }
...
# de-vectorisation, turn delta_f back into a series of pooled kernel deltas, i.e. the opposite of "vectorisation" above.
   ...
   foreach my $q (0 .. $self->{c2_kernels} - 1) {
      ...
      foreach my $i (0 .. $self->{c2_size} - 1) {
         foreach my $j (0 .. $self->{c2_size} - 1) {
            say "self->{delta_b2}[$q][0] = " .  $self->{delta_b2}[$q][0] . " + " . $self->{delta_c2_sigmoid}[$q][$i][$j] if $self->{debug};
